...
\begin{frame}{The Bitmask Algorithm}
    ...
    \begin{itemize}[<+->]
        ...
        \item Get these bits with a bitwise-\textsc{And} with $2^{\lfloor\log_2n\rfloor + 1} - 1$: \onslide<+->{\begin{align*}\everymath{}
            \underbrace{
                \overbrace{0}^{
                    \hidewidth 2^{W - 1} \hidewidth 
                },\ldots,0
        ...
        \end{align*}}
        \item Every resulting number $x \And \left(2^{\lfloor\log_2n\rfloor + 1} - 1\right)$ is at most \onslide<+->{$2^{\lfloor\log_2n\rfloor + 1} - 1 < 2n$}
    \end{itemize}
\end{frame}
