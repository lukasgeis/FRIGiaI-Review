...
%%% Array Drawing with Tikz instead of Equation
\iffalse
...
\begin{frame}{The Java Algorithm}
    ...
    \begin{itemize}
        ...
        \end{align*}}
        \item<8-> Only numbers in the \ER{leftover} interval get mapped to $\ER{2^W - \RNW} > 2^W - n$
        \item<9-> Algorithm: \begin{itemize}
            \item[(1)]<10-> Draw $x \in [0,2^W)$ and compute $r = x \Mod n$
            \item[(2)]<11-> Return $r$ if $x - r > 2^W - n$ else goto \textbf{\GB{(1)}}
        \end{itemize}
    \end{itemize}
\end{frame}
